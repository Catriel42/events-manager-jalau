    ...
    <div class="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-700 my-8 md:my-12">
      ...
      <div class="md:hidden space-y-4">
        @if (isLoading()) {
          ...
          @for (tag of tags(); track tag.id) {
            <div class="bg-[var(--bg-glass)] backdrop-blur-xl border border-[var(--border-color)] rounded-2xl p-4 flex justify-between items-center gap-4">
              <div class="min-w-0">
                <p class="text-[var(--text-primary)] font-medium truncate">{{ tag.name }}</p>
                <p class="text-xs text-[var(--text-secondary)] font-mono truncate mt-1">{{ tag.slug }}</p>
              </div>
